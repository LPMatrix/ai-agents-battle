...
def _who_threatens_me(me: Any, enemies: list[Any]) -> list[Any]:
...
def _focus_target(me: Any, enemies: list[Any], game_ctx: Any | None) -> Any | None:
    if not enemies:
        return None
    if game_ctx is not None and game_ctx.i_am_behind_leader():
        return max(enemies, key=lambda e: e.score)
    return min(enemies, key=lambda e: e.score)
...
def _finalize(
    ...
    if out:
        return out
    focus = _focus_target(me, enemies, game_ctx)
...
def decide(valid_actions: Any, game_ctx: Any | None = None) -> list[dict[str, Any]]:
    ...
    if len(picked) >= 2:
        ...
        return _finalize(actions, valid_actions, me, enemies, game_ctx)
    ...
    if len(picked) == 1:
        ...
        if threatened_by and unrelated:
            flee = _best_escape(me, enemies, valid_actions, remaining_die, game_ctx)
            if flee:
                actions.append(flee)
        else:
            focus = _focus_target(me, enemies, game_ctx)
    ...
    if threatened_by:
        for first, second in ((1, 2), (2, 1)):
            e1 = _best_escape(me, enemies, valid_actions, first, game_ctx)
            if not e1:
                continue
            actions.append(e1)
            ...
            e2 = _best_escape_from(nx, ny, enemies, valid_actions, second, game_ctx)
            if e2:
                actions.append(e2)
            return _finalize(actions, valid_actions, me, enemies, game_ctx)
        return _finalize(actions, valid_actions, me, enemies, game_ctx)
...
def _best_escape(me: Any, enemies: list[Any], valid_actions: Any, die_index: int, game_ctx: Any | None):
    return _best_escape_from(me.x, me.y, enemies, valid_actions, die_index, game_ctx)
